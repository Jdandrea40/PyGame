...
class Dog(Agent):
	...
	def calculatePathToNewTarget(self, target):
		...
		center = self.graph.getNodeFromPoint(self.center)
		targetCent = self.graph.getNodeFromPoint(target)
		if self.searchType == SearchType.BREADTH:
			self.path = self.graph.findPath_Breadth(center, targetCent)
		elif self.searchType == SearchType.DJIKSTRA:
			self.path = self.graph.findPath_Dijkstra(center, targetCent)
		elif self.searchType == SearchType.BEST:
			self.path = self.graph.findPath_BestFirst(center, targetCent)
		elif self.searchType == SearchType.A_STAR:
			self.path = self.graph.findPath_AStar(center, targetCent)
